/*
  U1 Argus Remote RFID - ESP32-S3 N16R8 V1.3
...
static const char* FW_VERSION = "V1.3";
...
static String configPageHtml(const String& msg) {
  ...
  body += F("<style>.otaProgress{margin-top:12px}.otaProgress progress{display:block;width:100%;height:16px;margin:8px 0;accent-color:var(--accent2)}.otaProgressText{display:block;font-weight:700;color:var(--accent2)}button:disabled{cursor:wait;opacity:.55}</style>");
  ...
  body += F("<div class='group'><p><b>Signed Firmware Update</b><br><small>");
  ...
  body += F("<button id='otaCheck' type='button' class='btn alt' data-current='");
  body += FW_VERSION;
  body += F("' data-catalog='");
  body += OTA_UPDATE_CATALOG_URL;
  body += F("'>Check for updates</button><small id='otaCheckText' class='help'>Checking for ESP32-S3 updates...</small><div id='otaRelease' class='msg' hidden></div>");
  if (otaUploadStatus.length()) {
    body += F("<div class='msg'><b>");
    body += htmlEscape(otaUploadStatus);
    body += F("</b></div>");
  }
  body += F("<form id='otaUploadForm' method='POST' action='/firmware_update' enctype='multipart/form-data'>");
  body += F("<input type='hidden' id='otaSize' name='size' value='0'>");
  body += F("<input type='file' id='otaFile' name='firmware' accept='.bin' required>");
  body += F("<button id='otaSubmit' type='submit' class='btn alt'>Install signed update</button></form>");
