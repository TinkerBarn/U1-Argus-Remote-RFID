/*
  U1 Argus Remote RFID - ESP32-S3 N16R8 V1.0
...
uint32_t lastFeederInfoQueryMs[READER_COUNT] = {0, 0};
...
static void loadSettings() {
  ...
  for (uint8_t i = 0; i < REMOTE_READER_COUNT; i++) {
    ...
    String leftToolKey = String("reader_tool_") + String(i + 2) + "_left";
    String rightToolKey = String("reader_tool_") + String(i + 2) + "_right";
...
static void scheduleNextMotionQuery(uint32_t now) {
  nextMotionQueryMs[activeReaderIndex] = now + PRINTER_MOTION_QUERY_MS + printerQueryJitterMs();
}
...
static String dashboardPageHtml() {
  ...
  body += String(F("<section class='grid'><article class='panel'><div class='cardTitle'><div><h2>Network</h2><p class='stamp'>")) + BOARD_LABEL + F(" status</p></div><span class='chip accent' id='revBadge'>rev -</span></div><div class='kv' id='netKv'></div><p class='foot'>Firmware ");
  body += FW_VERSION;
...
static void processPrinterPolling() {
  ...
  if ((int32_t)(now - nextMotionQueryMs[activeReaderIndex]) >= 0) {
    ...
    if (motionOk && gPrinterState.motionKnown && gPrinterState.filamentDetected) {
      if (lastFeederInfoQueryMs[activeReaderIndex] == 0 || (now - lastFeederInfoQueryMs[activeReaderIndex]) >= PRINTER_FEEDER_INFO_REFRESH_MS) {
        lastFeederInfoQueryMs[activeReaderIndex] = now;
...
static void updateFeederActivityTransitions() {
  for (uint8_t reader = 0; reader < READER_COUNT; reader++) {
    ...
    if (current && !lastFeederMotionValue[reader]) {
      ...
      if (NFC_DIAG_SERIAL) {
        Serial.printf("[FEEDER %s] activity edge detected, NFC priority for %lu ms\n",
                      READER_LABELS[reader],
                      (unsigned long)FEEDER_PRIORITY_WINDOW_MS);
      }
    }
    lastFeederMotionValue[reader] = current;
  }
