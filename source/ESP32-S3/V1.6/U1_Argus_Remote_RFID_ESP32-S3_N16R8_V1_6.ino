/*
  U1 Argus Remote RFID - ESP32-S3 N16R8 V1.6
...
static const char* FW_VERSION = "V1.6";
static const char* BOARD_LABEL = "ESP32-S3 N16R8";
static const char* OTA_UPDATE_CATALOG_URL = "https://tinkerbarn.github.io/U1-Argus-Remote-RFID/updates/esp32-s3.json";
static const size_t OTA_SIGNATURE_BYTES = 512;
static const size_t OTA_APP_SLOT_BYTES = 0x640000;
...
static const uint32_t TAG_UNCHANGED_ACTIVE_MS = 3000;
static const uint32_t TAG_UNCHANGED_PAUSE_MS = 2000;
...
static const uint32_t PRINTER_WS_RECONNECT_MS = 5000;
static const uint32_t PRINTER_WS_STALE_MS = 20000;
static const uint32_t PRINTER_WS_PING_MS = 15000;
static const size_t PRINTER_WS_FRAME_BUFFER_BYTES = 8192;
...
static const uint32_t PRINTER_LOST_INFO_REPAIR_COOLDOWN_MS = 15000;
...
static const uint8_t PN532_BOOT_RECOVERY_ATTEMPTS = 2;
...
bool otaUploadSuccessful = false;
String otaUploadStatus;
size_t otaUploadExpectedBytes = 0;
size_t otaUploadReceivedBytes = 0;
...
struct StableTagThrottleState {
  bool valid = false;
  uint8_t uid[10] = {0};
  uint8_t uidLen = 0;
  String source;
  String content;
  uint32_t sameContentSinceMs = 0;
...
StableTagThrottleState stableTagThrottle[READER_COUNT];
...
WiFiClient printerWsClient;
bool printerWsConnected = false;
bool printerWsHandshakeDone = false;
bool printerWsSubscribed = false;
uint32_t printerWsLastActivityMs = 0;
uint32_t printerWsLastPingMs = 0;
uint32_t printerWsNextReconnectMs = 0;
uint32_t printerWsLastLogMs = 0;
uint32_t printerWsConnectAttempts = 0;
uint32_t printerWsConnectOk = 0;
uint32_t printerWsDisconnects = 0;
uint32_t printerWsHandshakeOk = 0;
uint32_t printerWsSubscribeSent = 0;
uint32_t printerWsLastTxBytes = 0;
uint32_t printerWsLastRxBytes = 0;
uint32_t printerWsFrameRxCount = 0;
uint32_t printerWsPingCount = 0;
uint8_t printerWsFrameBuf[PRINTER_WS_FRAME_BUFFER_BYTES];
size_t printerWsFrameLen = 0;
String printerWsHandshakeBuffer;
String printerWsLastError;
String printerWsLastStatusLine;
String printerWsLastHost;
String printerWsLastJsonKind;
String printerWsLastJsonId;
bool printerAcceptsCardType = true;
String lastSentFingerprint[READER_COUNT];
String lastObservedFingerprint[READER_COUNT];
uint32_t lastLostInfoRepairMs[READER_COUNT] = {0, 0};
...
static void repairLostPrinterInfoIfNeeded(const char* reason);
...
static void sendTextNoCache(int code, const String& body);
...
static bool initPn532Reader(uint8_t reader) {
  if (reader >= READER_COUNT) return false;
  ...
  uint32_t version = 0;
  for (uint8_t attempt = 0; attempt < PN532_BOOT_RECOVERY_ATTEMPTS && !version; attempt++) {
    if (attempt > 0) {
      // OTA restarts leave the PN532 powered. Recover by draining HSU and
      // toggling RF/session state before asking for firmware again.
      flushPn532Input();
      pn532ReleaseTargets();
  ...
  if (isDebugSerialEnabled()) {
    ...
    Serial.printf("[PN532 %s] firmware IC=0x%02lX ver=%lu rev=%lu support=0x%02lX\n",
                  ...
                  (version >> 8) & 0xFF,
                  version & 0xFF);
    Serial.printf("[PN532 %s] SAMConfig=%s passive_retries=0x%02X(%s) RF reset off=%s on=%s\n",
                  READER_LABELS[reader],
                  samOk ? "ok" : "failed",
  ...
  if (PN532_SWITCH_RF_FIELD_BETWEEN_READERS) {
    bool initialOffOk = pn532SetRfField(false);
    if (isDebugSerialEnabled()) {
      Serial.printf("[PN532 %s] initial RF field off %s\n", READER_LABELS[reader], initialOffOk ? "ok" : "failed");
    }
  }
  return true;
}
...
static void sendTextNoCache(int code, const String& body) {
  ...
  web.sendHeader("Pragma", "no-cache");
  web.sendHeader("Expires", "0");
  web.send(code, "text/plain; charset=utf-8", body);
}
...
static void updatePrinterStateFromStatus(JsonObject status, const String& endpoint, int httpCode) {
  if (status.isNull()) return;
  bool relevant = status["filament_detect"].is<JsonObject>() ||
                  status["filament_feed left"].is<JsonObject>() ||
                  status["filament_feed right"].is<JsonObject>();
  if (!relevant) {
    for (uint8_t channel = 0; channel < 4; channel++) {
      String sensorKey = String("filament_motion_sensor e") + String(channel) + "_filament";
      if (status[sensorKey].is<JsonObject>()) {
        relevant = true;
        break;
      }
    }
  ...
  for (uint8_t reader = 0; reader < READER_COUNT; reader++) {
    ...
    next.queryOk = true;
    next.httpCode = httpCode;
    next.lastQueryMs = now;
    next.endpoint = endpoint;
    next.error = "";

    const uint8_t channel = printerChannelIndex();
    if (!infoArr.isNull() && channel < infoArr.size()) {
      JsonObject info = infoArr[channel];
      next.hasInfo = !info.isNull();
      next.vendor = info["VENDOR"].is<const char*>() ? String(info["VENDOR"].as<const char*>()) : "";
      next.manufacturer = info["MANUFACTURER"].is<const char*>() ? String(info["MANUFACTURER"].as<const char*>()) : "";
      next.mainType = info["MAIN_TYPE"].is<const char*>() ? String(info["MAIN_TYPE"].as<const char*>()) : "";
      ...
      next.rawJson = "";
      serializeJson(info, next.rawJson);
      lastPrinterInfoQueryMs[activeReaderIndex] = now;
    }
...
static bool sendPrinterWsFrame(uint8_t opcode, const uint8_t* payload, size_t len) {
  if (!printerWsClient.connected() || len > 65535) return false;

  uint8_t header[4];
  size_t headerLen = 0;
  header[headerLen++] = (uint8_t)(0x80 | (opcode & 0x0F));
  if (len < 126) {
    header[headerLen++] = (uint8_t)(0x80 | len);
  } else {
  ...
  const size_t frameLen = headerLen + sizeof(mask) + len;
  uint8_t* frame = (uint8_t*)malloc(frameLen);
  if (!frame) {
    printerWsLastError = "frame alloc failed";
    return false;
  }

  memcpy(frame, header, headerLen);
  memcpy(frame + headerLen, mask, sizeof(mask));
  uint8_t* maskedPayload = frame + headerLen + sizeof(mask);
  for (size_t i = 0; i < len; i++) maskedPayload[i] = payload[i] ^ mask[i % 4];

  size_t written = printerWsClient.write(frame, frameLen);
  free(frame);
  printerWsLastTxBytes = written;
  if (written != frameLen) {
    printerWsLastError = String("frame write short ") + String(written) + "/" + String(frameLen);
    return false;
  }
  printerWsClient.flush();
  return true;
}

static bool sendPrinterWsText(const String& text) {
  return sendPrinterWsFrame(0x1, (const uint8_t*)text.c_str(), text.length());
}

static void logPrinterWebSocketThrottled(const char* message) {
  uint32_t now = millis();
  if (printerWsLastLogMs != 0 && (now - printerWsLastLogMs) < 30000) return;
  printerWsLastLogMs = now;
  Serial.printf("[WS] %s\n", message ? message : "status");
}

static void disconnectPrinterWebSocket(const char* reason) {
  bool wasReady = printerWsConnected || printerWsHandshakeDone;
  if (printerWsClient.connected()) printerWsClient.stop();
  if (printerWsConnected || printerWsHandshakeDone) printerWsDisconnects++;
  if (reason && reason[0]) printerWsLastError = reason;
  printerWsConnected = false;
  printerWsHandshakeDone = false;
  printerWsSubscribed = false;
  printerWsFrameLen = 0;
  printerWsHandshakeBuffer = "";
  printerWsLastPingMs = 0;
  printerWsNextReconnectMs = millis() + PRINTER_WS_RECONNECT_MS;
  if (reason && wasReady) {
    logPrinterWebSocketThrottled((String("disconnected: ") + reason).c_str());
  }
}

static String printerWsSubscribePayload() {
  JsonDocument doc;
  doc["jsonrpc"] = "2.0";
  doc["method"] = "printer.objects.subscribe";
  doc["id"] = 100;
  JsonObject objects = doc["params"]["objects"].to<JsonObject>();
  JsonArray info = objects["filament_detect"].to<JsonArray>();
  info.add("info");
  objects["filament_feed left"].to<JsonVariant>().set(nullptr);
  objects["filament_feed right"].to<JsonVariant>().set(nullptr);
  for (uint8_t channel = 0; channel < 4; channel++) {
    String key = String("filament_motion_sensor e") + String(channel) + "_filament";
    JsonArray fields = objects[key].to<JsonArray>();
    fields.add("filament_detected");
  }
  String out;
...
static void handlePrinterWsText(const String& text) {
  printerWsLastActivityMs = millis();
  JsonDocument doc;
  DeserializationError err = deserializeJson(doc, text);
  if (err) {
    printerWsLastJsonKind = "decode failed";
    printerWsLastJsonId = "";
    printerWsLastError = String("json decode failed: ") + err.c_str();
    debugPrint("[DEBUG] WS JSON decode failed");
    return;
  }

  JsonVariant id = doc["id"];
  printerWsLastJsonId = id.isNull() ? "" : id.as<String>();
  if (!doc["method"].isNull()) {
    printerWsLastJsonKind = doc["method"].as<String>();
  } else if (!doc["result"].isNull()) {
    printerWsLastJsonKind = "result";
  } else if (!doc["error"].isNull()) {
    printerWsLastJsonKind = "error";
  } else {
    printerWsLastJsonKind = "unknown";
  }

  JsonObject status = doc["result"]["status"];
  if (!status.isNull()) {
    if (!printerWsSubscribed) Serial.println("[WS] Moonraker subscribed");
    printerWsSubscribed = true;
    printerWsLastError = "";
    updatePrinterStateFromStatus(status, "ws://printer/websocket", 200);
    return;
  }

  if (!id.isNull()) {
    long numericId = id.as<long>();
    if (numericId == 100 && !doc["result"].isNull()) {
      if (!printerWsSubscribed) Serial.println("[WS] Moonraker subscribed");
      printerWsSubscribed = true;
      printerWsLastError = "";
      return;
    }
    if (numericId == 100 && !doc["error"].isNull()) {
      printerWsLastError = "subscribe rejected";
      debugPrint("[DEBUG] WS subscribe rejected");
      return;
    }
  }
  ...
  if (!params.isNull() && params.size() > 0) {
    JsonObject eventStatus = params[0];
    updatePrinterStateFromStatus(eventStatus, "ws://printer/websocket", 200);
  }
}

static bool connectPrinterWebSocket() {
  if (WiFi.status() != WL_CONNECTED || portalMode) return false;
  String host = resolvedPrinterHostForUrl();
  if (!host.length()) return false;

  printerWsConnectAttempts++;
  printerWsLastHost = host + ":" + String(gSettings.printerPort);
  printerWsLastError = "tcp connecting";
  if (!printerWsClient.connect(host.c_str(), gSettings.printerPort)) {
    printerWsNextReconnectMs = millis() + PRINTER_WS_RECONNECT_MS;
    printerWsLastError = "tcp connect failed";
    logPrinterWebSocketThrottled("connect failed, retrying");
    return false;
  }

  printerWsConnectOk++;
  printerWsClient.setTimeout(2);
  printerWsHandshakeBuffer = "";
  printerWsFrameLen = 0;
  printerWsHandshakeDone = false;
  printerWsSubscribed = false;
  printerWsConnected = true;
  printerWsLastActivityMs = millis();
  printerWsLastPingMs = millis();
  printerWsLastError = "waiting for handshake";

  printerWsClient.print(String("GET /websocket HTTP/1.1\r\n") +
                        "Host: " + host + ":" + String(gSettings.printerPort) + "\r\n" +
                        "Upgrade: websocket\r\n" +
                        "Connection: Upgrade\r\n" +
                        "Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n" +
                        "Sec-WebSocket-Version: 13\r\n\r\n");
  printerWsClient.flush();
  debugPrintf("[DEBUG] WS connecting to Moonraker %s:%u\n", host.c_str(), (unsigned)gSettings.printerPort);
  return true;
}

static void processPrinterWsFrameBuffer() {
  while (printerWsFrameLen >= 2) {
    uint8_t b0 = printerWsFrameBuf[0];
    uint8_t b1 = printerWsFrameBuf[1];
    ...
    if (payloadLen == 126) {
      if (printerWsFrameLen < 4) return;
      payloadLen = ((uint16_t)printerWsFrameBuf[2] << 8) | printerWsFrameBuf[3];
      headerLen = 4;
    } else if (payloadLen == 127) {
      disconnectPrinterWebSocket("unsupported large frame");
      return;
    }

    uint8_t mask[4] = {0, 0, 0, 0};
    if (masked) {
      if (printerWsFrameLen < headerLen + 4) return;
      memcpy(mask, printerWsFrameBuf + headerLen, 4);
      headerLen += 4;
    }

    if (payloadLen > PRINTER_WS_FRAME_BUFFER_BYTES || printerWsFrameLen < headerLen + payloadLen) return;

    uint8_t* payload = printerWsFrameBuf + headerLen;
    if (masked) {
      for (uint64_t i = 0; i < payloadLen; i++) payload[i] ^= mask[i % 4];
    }

    printerWsLastRxBytes = (uint32_t)payloadLen;
    printerWsFrameRxCount++;

    if (opcode == 0x1) {
      String text;
      text.reserve((size_t)payloadLen);
      for (uint64_t i = 0; i < payloadLen; i++) text += (char)payload[i];
      handlePrinterWsText(text);
    } else if (opcode == 0x8) {
      disconnectPrinterWebSocket("server close");
      return;
    } else if (opcode == 0x9) {
      sendPrinterWsFrame(0xA, payload, (size_t)payloadLen);
    } else if (opcode == 0xA) {
      printerWsLastError = "";
    }

    size_t frameLen = headerLen + (size_t)payloadLen;
    memmove(printerWsFrameBuf, printerWsFrameBuf + frameLen, printerWsFrameLen - frameLen);
    printerWsFrameLen -= frameLen;
  }
}

static void maintainPrinterWebSocket() {
  if (portalMode || WiFi.status() != WL_CONNECTED || otaUploadActive) {
    if (printerWsConnected) disconnectPrinterWebSocket("inactive");
    return;
  }

  uint32_t now = millis();
  if (!printerWsConnected || !printerWsClient.connected()) {
    printerWsConnected = false;
    if ((int32_t)(now - printerWsNextReconnectMs) >= 0) {
      connectPrinterWebSocket();
    }
    return;
  }

  if (!printerWsHandshakeDone) {
    while (printerWsClient.available()) {
      char c = (char)printerWsClient.read();
      printerWsHandshakeBuffer += c;
      if (printerWsHandshakeBuffer.indexOf("\r\n\r\n") >= 0) {
        int lineEnd = printerWsHandshakeBuffer.indexOf("\r\n");
        printerWsLastStatusLine = lineEnd > 0 ? printerWsHandshakeBuffer.substring(0, lineEnd) : printerWsHandshakeBuffer;
        if (!printerWsHandshakeBuffer.startsWith("HTTP/1.1 101")) {
          printerWsLastError = "handshake failed";
          disconnectPrinterWebSocket("handshake failed");
          return;
        }
        printerWsHandshakeOk++;
        printerWsHandshakeDone = true;
        printerWsHandshakeBuffer = "";
        printerWsLastActivityMs = now;
        if (sendPrinterWsText(printerWsSubscribePayload())) {
          printerWsSubscribeSent++;
          printerWsLastError = "subscribe sent";
          debugPrint("[DEBUG] WS Moonraker subscribe sent");
        } else {
          disconnectPrinterWebSocket("subscribe send failed");
        }
        break;
      }
      if (printerWsHandshakeBuffer.length() > 1024) {
        printerWsLastError = "handshake overflow";
        disconnectPrinterWebSocket("handshake overflow");
        return;
      }
    }
    return;
  ...
  while (printerWsClient.available()) {
    if (printerWsFrameLen >= PRINTER_WS_FRAME_BUFFER_BYTES) {
      disconnectPrinterWebSocket("frame overflow");
      return;
    }
    int c = printerWsClient.read();
    if (c < 0) break;
    printerWsFrameBuf[printerWsFrameLen++] = (uint8_t)c;
    processPrinterWsFrameBuffer();
    if (!printerWsConnected) return;
  }

  if (printerWsHandshakeDone && (now - printerWsLastPingMs) > PRINTER_WS_PING_MS) {
    uint8_t pingPayload[1] = {0};
    if (sendPrinterWsFrame(0x9, pingPayload, sizeof(pingPayload))) {
      printerWsPingCount++;
      printerWsLastPingMs = now;
    } else {
      disconnectPrinterWebSocket("ping send failed");
      return;
    }
  }
...
static bool printerWebSocketReady() {
  return printerWsConnected && printerWsHandshakeDone && printerWsSubscribed && printerWsClient.connected();
}
...
static bool isNoneLikeValue(const String& value) {
...
static bool printerChannelLooksCleared() {
  if (!gPrinterState.queryOk || !gPrinterState.hasInfo) return false;

  return isNoneLikeValue(gPrinterState.vendor) &&
         isNoneLikeValue(gPrinterState.manufacturer) &&
         isNoneLikeValue(gPrinterState.mainType) &&
         isNoneLikeValue(gPrinterState.subType) &&
         gPrinterState.cardUidCsv.length() == 0 &&
         (gPrinterState.minTemp <= 0) &&
         (gPrinterState.maxTemp <= 0) &&
         (gPrinterState.bedTemp <= 0);
}

static bool canRepairLostPrinterInfo() {
  if (!gTagState.hasData || !gTagState.mappedPayload.length() || !gTagState.fingerprint.length()) return false;
  if (lastSentFingerprint[activeReaderIndex] != gTagState.fingerprint) return false;
  if (!gWebhookState.known || !gWebhookState.ok) return false;
  if (gPrinterState.lastQueryMs == 0 || (millis() - gPrinterState.lastQueryMs) > PRINTER_WS_STALE_MS) return false;
  return printerChannelLooksCleared();
}

static void repairLostPrinterInfoIfNeeded(const char* reason) {
  if (portalMode || WiFi.status() != WL_CONNECTED) return;
  if (!canRepairLostPrinterInfo()) return;

  uint32_t now = millis();
  if (lastLostInfoRepairMs[activeReaderIndex] != 0 &&
      (now - lastLostInfoRepairMs[activeReaderIndex]) < PRINTER_LOST_INFO_REPAIR_COOLDOWN_MS) {
    return;
  }

  lastLostInfoRepairMs[activeReaderIndex] = now;
  Serial.printf("[API] SET repair reason=%s reader=%s filament_detected=%d\n",
                reason ? reason : "lost printer info",
                READER_LABELS[activeReaderIndex],
                (gPrinterState.motionKnown && gPrinterState.filamentDetected) ? 1 : 0);
  if (sendFilamentSetPayload(gTagState.mappedPayload, gTagState.fingerprint, "repair lost printer info", true)) {
...
static bool allowTagSetWithFreshFilamentState() {
  bool motionOk = gPrinterState.motionKnown &&
                  gPrinterState.lastQueryMs != 0 &&
                  (millis() - gPrinterState.lastQueryMs) <= PRINTER_WS_STALE_MS;
  if (!motionOk && !printerWebSocketReady()) {
    motionOk = fetchPrinterMotionState();
...
static void handleStateApi() {
  ...
  JsonObject ws = doc["printer_ws"].to<JsonObject>();
  ws["connected"] = printerWebSocketReady();
  ws["handshake"] = printerWsHandshakeDone;
  ws["subscribed"] = printerWsSubscribed;
  ws["age_ms"] = printerWsLastActivityMs ? (millis() - printerWsLastActivityMs) : 0;
  ws["stale"] = !printerWebSocketReady() && (printerWsLastActivityMs == 0 || (millis() - printerWsLastActivityMs) > PRINTER_WS_STALE_MS);
  ws["last_error"] = printerWsLastError;
  ws["last_status"] = printerWsLastStatusLine;
  ws["last_host"] = printerWsLastHost;
  ws["connect_attempts"] = printerWsConnectAttempts;
  ws["connect_ok"] = printerWsConnectOk;
  ws["disconnects"] = printerWsDisconnects;
  ws["handshake_ok"] = printerWsHandshakeOk;
  ws["subscribe_sent"] = printerWsSubscribeSent;
  ws["tx_bytes"] = printerWsLastTxBytes;
  ws["rx_bytes"] = printerWsLastRxBytes;
  ws["rx_frames"] = printerWsFrameRxCount;
  ws["ping_count"] = printerWsPingCount;
  ws["last_json_kind"] = printerWsLastJsonKind;
  ws["last_json_id"] = printerWsLastJsonId;
...
static String configPageHtml(const String& msg) {
  String body;
  const bool fullSetup = !portalMode;
  body.reserve(fullSetup ? 15000 : 8500);
  body += F("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>");
  body += F("<title>U1 Argus RFID Setup</title><style>:root{--bg:#f4efe5;--panel:#fffaf1;--card:#fffdf8;--ink:#18231e;--muted:#5d6a62;--line:#d8cdb7;--strong:#bda982;--accent:#b85c38;--accent2:#2f5d50}*{box-sizing:border-box}body{font-family:'Trebuchet MS',Verdana,sans-serif;max-width:840px;margin:0 auto;padding:18px 14px 48px;background:radial-gradient(circle at top,#fff9ee 0,#f4efe5 48%,#ece3d3 100%);color:var(--ink)}.shell{background:var(--panel);border:1px solid var(--line);border-radius:22px;padding:22px;box-shadow:0 18px 44px rgba(67,49,26,.10)}h2,h3,p{margin-top:0}label{display:block;margin-top:10px;font-weight:700}input,select{width:100%;padding:10px 12px;margin-top:5px;border:1px solid #cdbfa6;border-radius:10px;background:#fffdf8}input[type=checkbox]{width:auto;margin:0 8px 0 0}.checkRow{display:flex;align-items:center;margin-top:8px;font-weight:700}.warn{display:block;margin-top:8px;padding:9px 11px;border-radius:10px;background:#fff2d6;border:1px solid #e3bd72;color:#6d4a14;line-height:1.35}.note{display:block;margin-top:10px;padding:9px 11px;border-radius:10px;background:#edf4ef;border:1px solid #c8dbcf;color:#2f5d50;line-height:1.35}button,.btn{display:inline-flex;align-items:center;justify-content:center;min-height:34px;margin:0;padding:8px 12px;border:none;border-radius:10px;background:var(--accent);color:#fff;text-decoration:none;font-weight:700;font-size:.92rem;line-height:1.1}.btn.alt,button.alt{background:var(--accent2)}.btn.soft,button.soft{background:#e9dcc7;color:#30463d}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.actions button,.actions .btn{margin-top:0}small,.muted{color:var(--muted)}code{background:#efe4d0;padding:2px 6px;border-radius:6px}.help{display:block;margin-top:5px;line-height:1.35}.top{display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap}.topRight{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.msg{padding:11px 13px;border-radius:11px;background:#f7e9d4;border:1px solid #e7d0ac;margin:14px 0}.group{margin-top:16px;padding:14px;border:1px solid var(--strong);border-left:5px solid var(--accent2);border-radius:14px;background:var(--card);box-shadow:0 4px 14px rgba(67,49,26,.04)}.group h3{font-size:1.02rem;margin:0 0 6px;padding-bottom:7px;border-bottom:1px solid #eadfca}.groupLead{margin-bottom:8px}.inlineForm{margin-top:10px;padding-top:8px;border-top:1px dashed #e0d2b8}.inlineForm input{margin-bottom:8px}.otaProgress{margin-top:12px}.otaProgress progress{display:block;width:100%;height:14px;margin:8px 0;accent-color:var(--accent2)}.otaProgressText{display:block;font-weight:700;color:var(--accent2)}button:disabled{cursor:wait;opacity:.55}@media(max-width:640px){.shell{padding:16px}.actions button,.actions .btn{width:100%;text-align:center}}</style></head><body>");
  ...
  body += F("</h2></div>");
  if (fullSetup) body += String(F("<a class='btn alt' href='/'>")) + "Dashboard" + F("</a>");
  body += F("</div></div>");
  if (msg.length()) {
  ...
  body += F("<div class='group'><h3>Network</h3><p class='groupLead'><small>Connect the reader to the same 2.4 GHz Wi-Fi network as the printer.</small></p>");
  ...
  body += F("</small>");
  body += F("</div><div class='group'><h3>Reader Address</h3>");
  ...
  body += F("</div><div class='group'><h3>Printer Connection</h3>");
  ...
  body += F("</div>");
  if (fullSetup) {
  body += F("<div class='group'><h3>Diagnostics</h3><label class='checkRow'><input type='checkbox' name='debug_serial' value='1'");
  ...
  body += F("<div class='group'><h3>Local Reader Mapping</h3>");
  for (uint8_t reader = 0; reader < READER_COUNT; reader++) {
  ...
  body += F("</div>");
  if (fullSetup) {
  body += String(F("<div class='group'><h3>")) + "Additional U1 Argus Remote Reader" + F("</h3><p class='groupLead'><small>") + "Optional: IP or full URL plus the two Tool Heads handled by the other two-spool reader." + F("</small></p>");
  body += String(F("<div class='actions'><button type='button' class='alt' id='prefillReaders'>")) + "Prefill RFID readers from mDNS name" + F("</button></div><small class='help'>");
  ...
  if (!fullSetup) {
    body += F("<div class='group'><small class='note'>QIDI list upload and signed firmware update are available from the normal dashboard Setup page after Wi-Fi is configured.</small></div>");
  }
  body += String(F("<div class='actions'><button type='submit' class='alt'>")) + "Save & Reboot" + F("</button>");
  if (fullSetup) body += String(F("<a class='btn soft' href='/'>")) + "Cancel" + F("</a>");
  body += F("</div></form>");
  if (fullSetup) {
  body += F("<div class='group'><h3>QIDI Tag Support</h3><p class='groupLead'><small>");
  ...
  body += F("</small></p><form class='inlineForm' method='POST' action='/qidi_cfg' enctype='multipart/form-data'>");
  body += F("<input type='file' name='cfg' accept='.cfg,text/plain'><div class='actions'><button type='submit' class='alt'>Upload QIDI cfg</button></div></form>");
  body += F("<form class='inlineForm' method='POST' action='/qidi_cfg_reset'><div class='actions'><button type='submit' class='soft'>Reset QIDI cfg</button></div></form></div>");
  body += F("<div class='group'><h3>Signed Firmware Update</h3><p class='groupLead'><small>");
  body += "Install only an ESP32-S3 signed OTA file published for this device. The signature is checked before the new firmware can boot.";
  body += F("</small></p><p><small>Current firmware: <code>");
  body += FW_VERSION;
  body += F("</code> | Update format: <code>*.ota.signed.bin</code></small></p>");
  body += F("<div class='actions'><button id='otaCheck' type='button' class='alt' data-current='");
  body += FW_VERSION;
  body += F("' data-catalog='");
  body += OTA_UPDATE_CATALOG_URL;
  body += F("'>Check for updates</button></div><small id='otaCheckText' class='help'>Checking for ESP32-S3 updates...</small><div id='otaRelease' class='msg' hidden></div>");
  if (otaUploadStatus.length()) {
    body += F("<div class='msg'><b>");
    body += htmlEscape(otaUploadStatus);
    body += F("</b></div>");
  }
  body += F("<form id='otaUploadForm' class='inlineForm' method='POST' action='/firmware_update' enctype='multipart/form-data'>");
  body += F("<input type='hidden' id='otaSize' name='size' value='0'>");
  body += F("<input type='file' id='otaFile' name='firmware' accept='.bin' required>");
  body += F("<div class='actions'><button id='otaSubmit' type='submit' class='alt'>Install signed update</button></div></form>");
  body += F("<div id='otaProgress' class='otaProgress' hidden><progress id='otaProgressBar' max='100' value='0'></progress><small id='otaProgressText' class='otaProgressText'>Ready.</small></div>");
  body += F("<small class='warn'>The reader pauses tag scanning while installing. A damaged, unsigned, or wrong firmware file is rejected and the running firmware remains active.</small></div>");
  body += String(F("<p><small>")) + "API target: " + F("<code>/printer/filament_detect/set</code></small></p>");
  body += F("<script>function u1aCleanHost(v){v=(v||'').trim();v=v.replace(/^https?:\\/\\//i,'');v=v.split('/')[0];v=v.split(':')[0];v=v.replace(/\\.local$/i,'');return v;}function u1aRemainingTools(){const local=[...document.querySelectorAll('[name^=channel_]')].map(x=>Number(x.value)+1);return[1,2,3,4].filter(x=>!local.includes(x));}function u1aSyncRemoteTools(){const other=u1aRemainingTools();for(const sel of document.querySelectorAll('[name^=reader_tool_]')){const keep=Number(sel.value);sel.innerHTML='<option value=\"0\">Not assigned</option>'+other.map(t=>`<option value=\"${t}\">Tool Head ${t}</option>`).join('');if(other.includes(keep))sel.value=String(keep);}}function u1aPrefillReaders(){const host=u1aCleanHost(document.querySelector('[name=hostname]').value);if(!host)return;const other=u1aRemainingTools();const input=document.querySelector('[name=reader_2]');if(input&&!input.value.trim())input.value='http://'+host+'-other.local';const left=document.querySelector('[name=reader_tool_2_left]');const right=document.querySelector('[name=reader_tool_2_right]');if(left&&other[0]&&(!left.value||left.value==='0'))left.value=String(other[0]);if(right&&other[1]&&(!right.value||right.value==='0'))right.value=String(other[1]);}for(const sel of document.querySelectorAll('[name^=channel_]'))sel.addEventListener('change',u1aSyncRemoteTools);const prefillBtn=document.getElementById('prefillReaders');if(prefillBtn)prefillBtn.addEventListener('click',u1aPrefillReaders);const otaForm=document.getElementById('otaUploadForm');if(otaForm)otaForm.addEventListener('submit',e=>{const f=document.getElementById('otaFile').files[0];if(!f){e.preventDefault();return;}if(!f.name.endsWith('.ota.signed.bin')){e.preventDefault();alert('Select a signed ESP32-S3 OTA file ending in .ota.signed.bin.');return;}if(!confirm('Install the signed firmware update and reboot the reader?')){e.preventDefault();return;}document.getElementById('otaSize').value=String(f.size);});</script>");
  body += F("<script>(()=>{const form=document.getElementById('otaUploadForm');if(!form)return;const file=document.getElementById('otaFile');const size=document.getElementById('otaSize');const btn=document.getElementById('otaSubmit');const box=document.getElementById('otaProgress');const bar=document.getElementById('otaProgressBar');const text=document.getElementById('otaProgressText');const status=(pct,msg)=>{box.hidden=false;bar.value=pct;text.textContent=msg;};const waitForReader=async()=>{status(100,'Firmware verified. Restarting reader and reconnecting...');for(let attempt=0;attempt<45;attempt++){await new Promise(resolve=>setTimeout(resolve,1000));try{const response=await fetch('/setup?ota_reconnect='+Date.now(),{cache:'no-store'});if(response.ok){status(100,'Reader is online. Reloading setup...');setTimeout(()=>location.reload(),400);return;}}catch(error){}}status(100,'Restart is taking longer than expected. Reload this page when the reader is online.');btn.disabled=false;};form.addEventListener('submit',event=>{event.preventDefault();event.stopImmediatePropagation();const selected=file.files[0];if(!selected)return;if(!selected.name.endsWith('.ota.signed.bin')){alert('Select a signed ESP32-S3 OTA file ending in .ota.signed.bin.');return;}if(!confirm('Install the signed firmware update and reboot the reader?'))return;size.value=String(selected.size);const formData=new FormData(form);btn.disabled=true;status(1,'Preparing signed firmware upload...');const request=new XMLHttpRequest();request.open('POST','/firmware_update');request.upload.onprogress=upload=>{if(!upload.lengthComputable)return;const pct=Math.round(upload.loaded*95/upload.total);status(pct,'Uploading signed firmware... '+Math.round(upload.loaded*100/upload.total)+'%');if(upload.loaded===upload.total)status(96,'Upload complete. Verifying signature...');};request.onload=()=>{if(request.status>=200&&request.status<300){waitForReader();return;}status(0,request.responseText||'Update rejected.');btn.disabled=false;};request.onerror=()=>{if(bar.value>=95){waitForReader();return;}status(0,'Upload connection failed. The running firmware remains active.');btn.disabled=false;};request.send(formData);},true);})();</script>");
  body += F("<script>(()=>{const check=document.getElementById('otaCheck');if(!check)return;const text=document.getElementById('otaCheckText');const release=document.getElementById('otaRelease');const numbers=value=>String(value).replace(/^V/i,'').split('.').map(n=>Number(n)||0);const newer=(available,current)=>{const a=numbers(available),b=numbers(current);for(let i=0;i<Math.max(a.length,b.length);i++){if((a[i]||0)!==(b[i]||0))return(a[i]||0)>(b[i]||0);}return false;};const inspect=async()=>{check.disabled=true;release.hidden=true;text.textContent='Checking the published ESP32-S3 update catalog...';try{const response=await fetch(check.dataset.catalog+'?check='+Date.now(),{cache:'no-store'});if(!response.ok)throw new Error('catalog');const catalog=await response.json();if(newer(catalog.latest,check.dataset.current)&&catalog.ota&&catalog.ota.download_url){release.textContent='';const heading=document.createElement('b');heading.textContent='Update available: '+catalog.latest;release.appendChild(heading);release.appendChild(document.createElement('br'));const link=document.createElement('a');link.className='btn alt';link.href=catalog.ota.download_url;link.textContent='Download signed OTA file';link.setAttribute('download','');release.appendChild(link);release.hidden=false;text.textContent='Download the signed file, then select it below to install it safely.';}else{text.textContent='This reader is running the current published ESP32-S3 firmware.';}}catch(error){text.textContent='The update catalog is not reachable. A signed OTA file can still be installed manually.';}check.disabled=false;};check.addEventListener('click',inspect);setTimeout(inspect,500);})();</script>");
  }
  body += F("</div>");
  body += F("</body></html>");
  return body;
...
static String dashboardPageHtml() {
  ...
  body += String(F("<section class='grid'><article class='panel'><div class='cardTitle'><div><h2>Network</h2><p class='stamp'>")) + BOARD_LABEL + F(" status</p></div><span class='chip accent' id='revBadge'>rev -</span></div><div class='kv' id='netKv'></div><div class='bssidList'><div class='label'>Moonraker WebSocket</div><p class='stamp' id='wsStatus'>Connecting...</p></div><div class='bssidList'><div class='label'>Visible 2.4 GHz BSSIDs for this SSID</div><p class='stamp' id='bssidAge'>No scan result yet</p><div class='bssidRows' id='bssidRows'></div></div><p class='foot'>Firmware ");
  body += FW_VERSION;
...
static void handleOtaUpload() {
  if (otaUploadSuccessful) {
    sendTextNoCache(200, "Signed update verified. Rebooting into the new firmware...");
    delay(1200);
    ESP.restart();
    return;
  }
  const String message = otaUploadStatus.length() ? otaUploadStatus : String("Firmware update failed.");
  sendTextNoCache(400, message);
}
...
static void handleOtaUploadChunk() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    otaUploadSuccessful = false;
    otaUploadStatus = "";
    otaUploadReceivedBytes = 0;
    otaUploadExpectedBytes = (size_t)web.arg("size").toInt();

    const esp_partition_t* running = esp_ota_get_running_partition();
    const esp_partition_t* nextOta = esp_ota_get_next_update_partition(nullptr);
    if (!nextOta || nextOta == running || nextOta->size != OTA_APP_SLOT_BYTES) {
      otaUploadStatus = "OTA update unavailable: dual-slot firmware layout is not active.";
      return;
    }
    if (!upload.filename.endsWith(".ota.signed.bin")) {
      otaUploadStatus = "Rejected: select a signed ESP32-S3 OTA file ending in .ota.signed.bin.";
      return;
    }
    if (otaUploadExpectedBytes <= OTA_SIGNATURE_BYTES || otaUploadExpectedBytes > nextOta->size) {
      otaUploadStatus = "Rejected: signed firmware file size is invalid for this device.";
      return;
    }

    static UpdaterECDSAVerifier otaVerifier(OTA_PUBLIC_KEY, OTA_PUBLIC_KEY_LEN, HASH_SHA256);
    if (!Update.installSignature(&otaVerifier) || !Update.begin(otaUploadExpectedBytes, U_FLASH)) {
      otaUploadStatus = String("Unable to begin firmware update: ") + Update.errorString();
      return;
    }

    otaUploadActive = true;
    Serial.printf("[OTA] receiving signed update: %s, bytes=%lu, target=%s\n",
                  upload.filename.c_str(),
                  (unsigned long)otaUploadExpectedBytes,
                  nextOta->label);
  } else if (upload.status == UPLOAD_FILE_WRITE) {
    if (!otaUploadActive) return;
    if (Update.write(upload.buf, upload.currentSize) != upload.currentSize) {
      otaUploadStatus = String("Firmware write failed: ") + Update.errorString();
      Update.abort();
      otaUploadActive = false;
      return;
    }
    otaUploadReceivedBytes += upload.currentSize;
  } else if (upload.status == UPLOAD_FILE_END) {
    if (!otaUploadActive) return;
    otaUploadActive = false;
    if (upload.totalSize != otaUploadExpectedBytes || otaUploadReceivedBytes != otaUploadExpectedBytes) {
      Update.abort();
      otaUploadStatus = "Rejected: firmware upload was incomplete.";
      return;
    }
    if (!Update.end() || !Update.isFinished()) {
      otaUploadStatus = String("Rejected: firmware signature or image validation failed (") + Update.errorString() + ").";
      Serial.printf("[OTA] signed update rejected: %s\n", Update.errorString());
      return;
    }
    otaUploadSuccessful = true;
    otaUploadStatus = "Signed firmware verified successfully.";
    Serial.printf("[OTA] signed update verified: %lu bytes; reboot scheduled\n",
                  (unsigned long)otaUploadReceivedBytes);
  } else if (upload.status == UPLOAD_FILE_ABORTED) {
    if (otaUploadActive) Update.abort();
    otaUploadActive = false;
    otaUploadStatus = "Firmware update upload was aborted.";
    Serial.println("[OTA] signed update upload aborted");
  }
}
...
static void setupPortalRoutes() {
  ...
  web.on("/firmware_update", HTTP_POST, handleOtaUpload, handleOtaUploadChunk);
...
static bool buildFilamentDetectPayload(const OpenSpoolFields& fields,
  ...
  if (printerAcceptsCardType && cardType && cardType[0]) info["CARD_TYPE"] = cardType;
...
static bool buildCardUidOnlyPayload(const uint8_t* uid,
                                    uint8_t uidLen,
                                    const char* cardType,
                                    String& outPayload,
                                    String& outFingerprint) {
...
static bool stripCardTypeFromPayload(const String& payload, String& strippedPayload) {
  JsonDocument doc;
  DeserializationError err = deserializeJson(doc, payload);
  if (err) return false;

  JsonObject info = doc["info"];
  if (info.isNull() || info["CARD_TYPE"].isNull()) return false;
  info.remove("CARD_TYPE");

  strippedPayload = "";
  serializeJson(doc, strippedPayload);
  return strippedPayload.length() > 0;
}

static bool responseRejectsCardType(int code, const String& resp) {
  return code == 400 && resp.indexOf("unsupported fields: CARD_TYPE") >= 0;
}
...
static bool sendFilamentSetPayload(const String& payload, const String& fingerprint, const char* reason, bool pulseOnFail) {
  ...
  bool ok = postJson(filamentDetectUrl(), effectivePayload, code, resp);

  if (responseRejectsCardType(code, resp)) {
    printerAcceptsCardType = false;
    String strippedPayload;
    if (stripCardTypeFromPayload(payload, strippedPayload)) {
      Serial.println("[API] CARD_TYPE unsupported by printer firmware, retrying without CARD_TYPE");
      effectivePayload = strippedPayload;
      ok = postJson(filamentDetectUrl(), effectivePayload, code, resp);
    }
  }
...
static const char* cardTypeForTagSource(const char* source) {
  if (!source) return "";
  if (strcmp(source, "QIDI") == 0 || strcmp(source, "CARD_UID_M1") == 0) return "M1";
  if (strcmp(source, "OpenSpool") == 0 || strcmp(source, "CARD_UID_NTAG") == 0) return "NTAG";
  return "";
}

static void handleCardUidOnlyTagPayload(const char* source, const uint8_t* uid, uint8_t uidLen) {
  String payload;
  String fingerprint;
  const char* cardType = cardTypeForTagSource(source);
  if (!buildCardUidOnlyPayload(uid, uidLen, cardType, payload, fingerprint)) {
    pulseTagLedError();
    return;
  }

  OpenSpoolFields emptyFields;
  String uidCsv = uidBytesToCsv(uid, uidLen);
  lastTagSeenMs[activeReaderIndex] = millis();
  storeTagState(emptyFields, "", uid, uidLen, payload, fingerprint, source ? source : "CARD_UID");
  ...
  if (fingerprint != lastObservedFingerprint[activeReaderIndex]) {
    ...
    debugPrintf("[DEBUG] NFC UID-only tag UID %s payload %s\n", bytesToHexString(uid, uidLen).c_str(), payload.c_str());
  }

  bool printerInfoFresh = gPrinterState.queryOk &&
                          gPrinterState.lastQueryMs != 0 &&
                          (millis() - gPrinterState.lastQueryMs) <= PRINTER_TAG_INFO_MAX_AGE_MS;
  if (printerInfoFresh && gPrinterState.cardUidCsv == uidCsv) {
    lastSentFingerprint[activeReaderIndex] = fingerprint;
    clearPendingVerification();
    debugPrint("[DEBUG] Printer already has tag CARD_UID, webhook skipped");
    return;
  }
  ...
  if (sentBefore && gWebhookState.lastSentMs && (millis() - gWebhookState.lastSentMs) < WEBHOOK_RESEND_GRACE_MS) {
    debugPrint("[DEBUG] CARD_UID payload recently sent, waiting for printer confirmation");
    return;
  }

  if (sendFilamentSetPayload(payload, fingerprint, "card uid", true)) {
    clearPendingVerification();
    scheduleNextInfoSync(millis());
  }
}
...
static void handleValidMappedTagPayload(const char* source, const String& openspoolJson, const uint8_t* uid, uint8_t uidLen) {
  ...
  String payload;
  String fingerprint;
  if (!buildFilamentDetectPayload(fields, uid, uidLen, cardTypeForTagSource(source), payload, fingerprint)) {
    Serial.printf("[NFC] %s parse failed\n", source ? source : "tag");
...
static void resetStableTagThrottle(uint8_t reader) {
  if (reader >= READER_COUNT) return;
  StableTagThrottleState& state = stableTagThrottle[reader];
  state.valid = false;
  state.uidLen = 0;
  state.source = "";
  state.content = "";
  state.sameContentSinceMs = 0;
  state.pauseUntilMs = 0;
}

static bool stableTagThrottlePaused(uint8_t reader, uint32_t now) {
  if (reader >= READER_COUNT) return false;
  const StableTagThrottleState& state = stableTagThrottle[reader];
  return state.valid && state.pauseUntilMs != 0 && (int32_t)(now - state.pauseUntilMs) < 0;
}

static bool shouldPublishTagContent(uint8_t reader, const char* source, const String& content,
                                    const uint8_t* uid, uint8_t uidLen) {
  if (reader >= READER_COUNT || !source || !uid || uidLen == 0 || uidLen > sizeof(stableTagThrottle[0].uid)) {
    return true;
  }

  const uint32_t now = millis();
  StableTagThrottleState& state = stableTagThrottle[reader];
  const bool same = state.valid &&
                    state.uidLen == uidLen &&
                    memcmp(state.uid, uid, uidLen) == 0 &&
                    state.source == source &&
  ...
  if (!same) {
    ...
    state.source = source;
    state.content = content;
    state.sameContentSinceMs = now;
    state.pauseUntilMs = 0;
    return true;
  }

  const bool periodicCheckAfterPause = state.pauseUntilMs != 0 &&
                                        (int32_t)(now - state.pauseUntilMs) >= 0;
  if ((now - state.sameContentSinceMs) >= TAG_UNCHANGED_ACTIVE_MS) {
    state.pauseUntilMs = now + TAG_UNCHANGED_PAUSE_MS;
    if (NFC_DIAG_SERIAL) {
      Serial.printf("[NFC %s] unchanged %s tag remains present; pausing reads for %lu ms\n",
                    READER_LABELS[reader],
                    source,
                    (unsigned long)TAG_UNCHANGED_PAUSE_MS);
    }
  }

  // Suppress rapid duplicate queue traffic. Once reads have been throttled,
  // forward one periodic confirmation so a missed SET or later printer
  // mismatch can still be repaired while the spool remains parked.
  return periodicCheckAfterPause;
}
...
static void processPrinterPolling() {
  ...
  if (!printerWebSocketReady() && (int32_t)(now - nextMotionQueryMs[activeReaderIndex]) >= 0) {
  ...
  if ((int32_t)(now - nextInfoSyncMs[activeReaderIndex]) >= 0) {
    if (fetchPrinterInfoState("slow sync")) {
      repairLostPrinterInfoIfNeeded("slow sync");
    }
    scheduleNextInfoSync(millis());
  }
}
...
static void processQueuedNfcTags() {
  ...
  while (xQueueReceive(nfcTagQueue, &item, 0) == pdTRUE) {
    ...
    if (strncmp(item.source, "CARD_UID", 8) == 0) {
      handleCardUidOnlyTagPayload(item.source, item.uid, item.uidLen);
    } else {
...
static void nfcPollingTask(void* /*pvParam*/) {
  ...
  for (;;) {
    ...
    for (uint8_t reader = 0; reader < READER_COUNT; reader++) {
      ...
      if (stableTagThrottlePaused(reader, now)) {
        if (nfcFocusReader == (int8_t)reader) {
          releaseReaderFocusAfterReadAttempt();
        }
        continue;
      ...
      if (uidLen == 4) {
        ...
        if (hasQidiTag && shouldPublishTagContent(reader, "QIDI", qidiJson, uid, uidLen)) {
        ...
        releaseReaderFocusAfterReadAttempt();
        if (hasValidTag && shouldPublishTagContent(reader, "OpenSpool", openspoolJson, uid, uidLen)) {
...
// ============================== Setup / Loop ==============================
static void logStorageDiagnostics() {
  const uint32_t flashBytes = ESP.getFlashChipSize();
  const uint32_t psramBytes = ESP.getPsramSize();
  const uint32_t sketchBytes = ESP.getSketchSize();
  const esp_partition_t* running = esp_ota_get_running_partition();
  const esp_partition_t* nextOta = esp_ota_get_next_update_partition(nullptr);

  Serial.printf("[MEMORY] detected flash=%lu bytes (%lu MB), PSRAM=%lu bytes (%lu MB)\n",
                (unsigned long)flashBytes,
                (unsigned long)(flashBytes / (1024UL * 1024UL)),
                (unsigned long)psramBytes,
                (unsigned long)(psramBytes / (1024UL * 1024UL)));
  Serial.printf("[MEMORY] sketch=%lu bytes, OTA target capacity=%lu bytes\n",
                (unsigned long)sketchBytes,
                (unsigned long)ESP.getFreeSketchSpace());

  if (running) {
    Serial.printf("[PARTITION] running label=%s address=0x%06lX size=%lu bytes subtype=0x%02X\n",
                  running->label,
                  (unsigned long)running->address,
                  (unsigned long)running->size,
                  (unsigned)running->subtype);
  } else {
    Serial.println("[PARTITION] running application partition not found");
  }

  if (nextOta) {
    Serial.printf("[PARTITION] next OTA label=%s address=0x%06lX size=%lu bytes subtype=0x%02X\n",
                  nextOta->label,
                  (unsigned long)nextOta->address,
                  (unsigned long)nextOta->size,
                  (unsigned)nextOta->subtype);
  } else {
    Serial.println("[PARTITION] no alternate OTA application slot in current layout");
  }
  const bool dualOtaReady = running && nextOta && running != nextOta &&
                            running->size >= 0x600000UL && nextOta->size >= 0x600000UL;
  Serial.printf("[OTA] permanent 16 MB dual-slot layout=%s\n",
                dualOtaReady ? "ready" : "NOT READY");
  Serial.println("[OTA] signed updates enabled; unchanged stationary tags use periodic reads");
}
...
void loop() {
  ...
  for (uint8_t reader = 0; reader < READER_COUNT; reader++) {
    ...
    repairLostPrinterInfoIfNeeded("printer state");
  }
  selectReader(0);
}
