/*
  Release sketch: source/V1.3/U1_Argus_Remote_RFID_V1.3.ino
  Public release: V1.3

  U1 Argus Remote RFID - V1.3
  ---------------------------------
  ...
  Release changes (V1.3):
  - Keeps setup values in Preferences and adds a config-version marker for future migrations.
  - Allows the Snapmaker U1 printer address to be an IPv4 address or hostname/mDNS name.
  - Resolves .local printer names through mDNS before building printer API URLs.
  - Adds a setup-page prefill button for the three additional reader URLs based on this reader's mDNS name and Tool Head.
  ...
  - Webhook dedupe compares against the current queried printer channel state.
*/
...
static const char* FW_ITERATION = "V1.3";
...
Settings gSettings = {
  "", "", "u1-argus-rfid", "192.168.1.10", DEFAULT_PRINTER_PORT, 0, "en", {"", "", ""}, {0, 0, 0}
};
...
static String configPageHtml(const String& msg) {
  ...
  body += String(F("<label>")) + tr("Snapmaker U1 address", "Snapmaker U1 Adresse") + F("</label><input name='printer_ip' maxlength='63' required placeholder='192.168.1.120 or u1.local' value='"); body += htmlEscape(gSettings.printerIp); body += F("'><small class='help'>");
  body += tr("IP or mDNS hostname, e.g. 192.168.1.120 or u1.local", "IP oder mDNS-Hostname, z. B. 192.168.1.120 oder u1.local");
  body += F("</small>");
  ...
  body += String(F("<label>")) + tr("Tool Head", "Druckkopf") + F("</label><select name='channel' id='channel'>");
  for (int i = 0; i < 4; i++) {
    body += "<option value='" + String(i) + "'" + String(gSettings.channel == i ? " selected" : "") + ">";
    body += tr("Tool Head ", "Druckkopf ");
    body += String(i + 1);
    body += F("</option>");
  }
  body += F("</select>");
  body += String(F("<div class='group'><p><b>")) + tr("Additional U1 Argus Remote Readers", "Weitere U1 Argus Remote Reader") + F("</b><br><small>") + tr("Optional: IP or full URL plus Tool Head for up to three more dashboard buttons.", "Optional: IP oder komplette URL plus Druckkopf fuer bis zu drei weitere Dashboard-Buttons.") + F("</small></p>");
  body += String(F("<button type='button' class='btn alt' id='prefillReaders'>")) + tr("Prefill RFID readers from mDNS name", "RFID Reader aus mDNS-Namen vorbefuellen") + F("</button><br><small class='help'>");
  body += tr("Fills only empty reader fields and skips this reader's own Tool Head.", "Fuellt nur leere Reader-Felder und ueberspringt den eigenen Druckkopf.");
  body += F("</small>");
  for (uint8_t i = 0; i < REMOTE_READER_COUNT; i++) {
    body += String(F("<label>")) + tr("Reader ", "Reader ");
    body += String(i + 2);
    body += tr(" IP or URL", " IP oder URL");
    ...
    body += F(">");
    body += tr("Not assigned", "Nicht zugeordnet");
    body += F("</option>");
...
static String dashboardPageHtml() {
  ...
  String currentToolHead = String(tr("Tool Head ", "Druckkopf ")) + String(gSettings.channel + 1);
  body += F("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>");
  ...
  body += String(F("<section class='hero'><div class='panel'><h1>U1 Argus Remote RFID</h1><p class='sub'>")) + tr("Live status for the printer channel and the last OpenSpool tag that was read. The page only refreshes when the state actually changes.", "Live-Status fuer Drucker-Channel und den zuletzt gelesenen OpenSpool-Tag. Die Seite zieht nur neue Daten, wenn sich der Zustand wirklich geaendert hat.") + F("</p><div class='meta'><span class='chip'><span class='dot warn' id='wifiDot'></span><span id='wifiText'>") + tr("Checking Wi-Fi", "WLAN wird geprueft") + F("</span></span><span class='chip'><span class='dot ok'></span><span>URL: ");
  body += htmlEscape(dashboardUrl);
  body += String(F("</span></span><span class='chip'><span class='dot warn' id='printerDot'></span><span id='printerText'>")) + tr("Loading printer status", "Druckerstatus wird geladen") + F("</span></span><span class='chip'><span class='dot warn' id='tagDot'></span><span id='tagText'>") + tr("Loading tag status", "Tag-Status wird geladen") + F("</span></span></div></div><div class='panel actions'><div class='readerNav'>");
  body += F("<h1 style='width:100%'>");
  body += htmlEscape(currentToolHead);
  body += F("</h1>");
  body += langToggleHtml("/");
  body += String(F("<a class='btn' href='/setup'>")) + tr("Setup", "Setup") + F("</a>");
  for (uint8_t i = 0; i < REMOTE_READER_COUNT; i++) {
  ...
  body += String(F("<section class='grid'><article class='panel'><div class='cardTitle'><div><h2>")) + tr("Printer Tool Head", "Druckkopf") + F("</h2><p class='stamp' id='printerStamp'>") + tr("No response yet", "Noch keine Antwort") + F("</p></div><span class='chip'><span id='channelId'>-</span></span></div><div class='kv' id='printerKv'></div><div class='json' id='printerJson'>") + tr("No printer data yet.", "Noch keine Druckerdaten.") + F("</div></article><article class='panel'><div class='cardTitle'><div><h2>") + tr("Tag Reader", "Tag Reader") + F("</h2><p class='stamp' id='tagStamp'>") + tr("No valid tag read yet", "Noch kein gueltiger Tag gelesen") + F("</p></div><span class='chip' id='tagBadge'>") + tr("no tag", "kein Tag") + F("</span></div><div class='kv' id='tagKv'></div><div class='json' id='tagJson'>") + tr("No tag data yet.", "Noch keine Tag-Daten.") + F("</div></article></section>");
  ...
  body += FW_VERSION;
  if (strcmp(FW_VERSION, FW_ITERATION) != 0) {
    body += F(" / ");
    body += FW_ITERATION;
  }
...
static void setupPortalRoutes() {
  ...
  web.on("/save", HTTP_POST, []() {
    ...
    if (ch < 0 || ch > 3) {
      web.send(400, "text/html; charset=utf-8", configPageHtml(tr("Tool Head selection is invalid.", "Druckkopf-Auswahl ist ungueltig.")));
      return;
    }
    if (portVal == 0 || portVal > 65535 || !parseIpPort(printerIp.c_str(), (uint16_t)portVal)) {
      web.send(400, "text/html; charset=utf-8", configPageHtml(tr("Printer address/port is invalid.", "Printer Adresse/Port ist ungueltig.")));
      return;
    }
    ...
    ok += F("'><title>");
    ok += tr("Saved", "Gespeichert");
    ok += F("</title></head><body><h3>");
    ok += tr("Saved. Rebooting...", "Gespeichert. Neustart...");
    ok += F("</h3><p>");
    ok += tr("The dashboard should reopen automatically in about 10 seconds.", "Das Dashboard sollte in etwa 10 Sekunden automatisch wieder geoeffnet werden.");
    ok += F("</p><p><a href='");
    ok += htmlEscape(dashboardUrl);
    ok += F("'>");
    ok += tr("Open dashboard now", "Dashboard jetzt oeffnen");
    ok += F("</a></p></body></html>");
...
static bool buildDesiredPrinterComparableFingerprint(const String& openspoolJson,
                                                     const uint8_t* uid,
                                                     uint8_t uidLen,
  ...
  String vendor = src["brand"].is<const char*>() ? String(src["brand"].as<const char*>()) : "";
  String mainType = src["type"].is<const char*>() ? String(src["type"].as<const char*>()) : "";
  String subType = src["subtype"].is<const char*>() ? String(src["subtype"].as<const char*>()) : "";
  String colorHex = "";
  if (src["color_hex"].is<const char*>()) {
    int rgb = parseHexColorToRgbInt(src["color_hex"].as<const char*>());
    if (rgb >= 0) colorHex = rgbIntToHexString(rgb);
  }
  ...
  int bedTemp = -1;
  int iv = 0;
  if (tryParseIntField(src["min_temp"], iv)) minTemp = iv;
  if (tryParseIntField(src["max_temp"], iv)) maxTemp = iv;
  ...
  outFingerprint += '|';
  outFingerprint += vendor;
  outFingerprint += '|';
  outFingerprint += mainType;
  outFingerprint += '|';
  outFingerprint += subType;
  outFingerprint += '|';
  outFingerprint += colorHex;
  outFingerprint += '|';
  if (includeUid) outFingerprint += uidBytesToCsv(uid, uidLen);
  outFingerprint += '|';
  outFingerprint += String(minTemp);
  outFingerprint += '|';
  outFingerprint += String(maxTemp);
  outFingerprint += '|';
  outFingerprint += String(bedTemp);
  return true;
}
...
static bool printerStateMatchesTag(const String& openspoolJson, const uint8_t* uid, uint8_t uidLen) {
  if (!gPrinterState.queryOk || !gPrinterState.hasInfo) return false;

  String desired;
  if (!buildDesiredPrinterComparableFingerprint(openspoolJson, uid, uidLen, desired, gPrinterState.cardUidCsv.length() > 0)) return false;

  String current = currentPrinterComparableFingerprint();
  bool matches = desired == current;
  if (!matches) {
    debugPrintf("[DEBUG] Printer channel differs from tag, resend allowed. desired=%s current=%s\n",
                desired.c_str(),
                current.c_str());
  }
  return matches;
