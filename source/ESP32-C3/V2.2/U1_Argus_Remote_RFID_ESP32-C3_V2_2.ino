...
// Mirror logs to USB-Serial/JTAG when the board profile maps Serial to UART0.
...
static void scheduleNextMotionQuery(uint32_t now) {
  uint32_t intervalMs = (gPrinterState.motionKnown && gPrinterState.filamentDetected)
                          ? PRINTER_LOADED_MOTION_QUERY_MS
                          : PRINTER_MOTION_QUERY_MS;
  nextMotionQueryMs = now + intervalMs + printerQueryJitterMs();
}
...
void loop() {
  ...
  if (nfcReady &&
      !(gPrinterState.motionKnown && gPrinterState.filamentDetected) &&
      (now - lastPollMs) >= TAG_POLL_MS) {
    lastPollMs = now;
